...
public class ExprProcessor implements CodeConstants {
  ...
  private static final Map<Integer, Integer> functionMap = new HashMap<Integer, Integer>() {{
	put(opc_arraylength, FunctionExprent.FUNCTION_ARRAY_LENGTH);
    put(opc_checkcast, FunctionExprent.FUNCTION_CAST);
    put(opc_instanceof, FunctionExprent.FUNCTION_INSTANCEOF);
  }};
  ...
  public void processBlock(BasicBlockStatement stat, PrimitiveExpressionList data, StructClass cl) {
    ...
    for (int i = 0; i < seq.length(); i++) {
      ...
      Set<Integer> offsets = offset >= 0 ? map : null;
  ...
  public static List<ClassesProcessor.ClassNode> enclosingClassList() {
    ...
    List<ClassesProcessor.ClassNode> enclosingClassList = new ArrayList<ClassesProcessor.ClassNode>();
